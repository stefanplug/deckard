...
{
    echo "<table border=0><tr><td></td><td><span style='font-weight:bold'>Server:</span></td><td><span style='font-weight:bold'>" . $servers_row['hostname'] . "</span></td><td><span style='font-weight:bold'>" . $servers_row['v4'] . "</span></td></tr>";
    ...
    while($nodes_row = mysqli_fetch_array($nodes))
    {
        ...
        while($masters = mysqli_fetch_array($master_nodes))
        {
            ...
            if($updatetime < $staleout_time)
            {
                if($masters['active'] == 1)
                {
                    echo "<tr bgcolor='green'><td></td><td>Master node:</td><td>" . $masters['hostname'] . "</td><td>" . $masters['v4'] . "</td><td bgcolor='green'>UP " . $updatetime . " seconds ago</td></tr>";
                }
                else
                {
