...
while($servers_row = mysqli_fetch_array($servers))
{
    ...
    {
        $server_seen = mysqli_query($con,"SELECT machinestates.tstamp,machinestate.active FROM machines, machinestates WHERE machines.id = machinestates.slave_id AND machinestates.slave_id=" . $nodes_row['id'] ." AND machinestates.master_id=" . $servers_row['id']);
